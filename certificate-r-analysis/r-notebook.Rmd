---
title: "J. Tellew - DBMI 2019 Blockchain Certificate Database R Notebook"
output: html_notebook
---

# About
This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook created to analyze some results of my project in the UCSD DBMI 2019 summer internship program.

# Plotting the Data
I'll be making a plot (or several) over the next few sections using the data collected from experiments run on the AWS VM.

### Importing the Data
First, I'm going to import all of the `.csv` files in my data folder. Then, I'll check my list of files to make sure that everything was found properly.
```{r}
...
```
### Cleaning the Data
The data is recorded in the format `hh:mm:ss.sss` which does not play nicely with R. I'll be splitting the `time` column into three different numeric columns instead (`hours`, `minutes`, and `seconds`) to make it easier to plot.
```{r}
library(tidyr)
library(dplyr)
...
```
### Creating the Plot
Now, I'll go ahead and plot the data. My plot will be a line graph with each of the trials (each represented by a `.csv` file) plotted separately on the same graph.
```{r}
library(ggplot2)
# Create the plot
combinedPlot = ggplot()
...
```
### Adding a Mean Line
Now, I'll add a line to represent the mean so we can reduce clutter on the graph. The mean will be calculated by adding the values at each certificate interval together across all of the data frames, then dividing by the number of trials.
```{r}
...
# Comparing Mean Lines
Now I'm going to make a plot comparing the mean times between experiments running the old system with PDFs, the new system with PDFs, and the new system without PDFs.

### Calculating the Mean for the Strawman Experiment With PDFs
```{r}
# *** LOAD THE DATA ***
...
### Calculating the Mean for the Strawman Experiment Without PDFs
...
### Calculating the Mean for the Current System Without PDFs
...
I'm going to create a data frame with the total certificates and the mean values from each of the three experiments so I can graph it with ggplot2
```{r}
...
### Plotting the Data Frame (Zoomed In)
Now I'll plot the same data but without the line `Strawman Mean (With PDF)` from the previous graph so we can see the other three lines better to compare them.
```{r}
library(ggplot2)
...
# Conclusion
I plotted several trials on the same graph and then found the mean. Then, I plotted the mean for 4 different experiments and compared them with two different graphs. I also showed some basic ggplot2 customizations for your graphs. Hopefully this has been a helpful explaination of how I created the graphs for this project. Thanks for reading!